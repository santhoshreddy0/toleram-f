...
function ScoreDashboard({ team }) {
    ...
    const getPlayerData = (player) => [
        ...
        { content: player.stats.ballsPlayed },
        { content: player.stats.points },
        { content: player.stats.fours },
        { content: player.stats.sixes },
        { content: player.stats.wickets },
        { content: player.stats.maidenOvers },
        { content: player.stats.stumps },
        { content: player.stats.catches },
        { content: player.stats.runOuts },
    ];
                                    ...
                                    team?.players?.map((player) => (
                                        <tr key={player.id}>
                                            {getPlayerData(player).map((cell, index) => (
                                                <td key={index} className="whitespace-nowrap px-2 py-2 text-sm text-gray-100">
                                                    {cell.content}
                                                </td>
                                            ))}

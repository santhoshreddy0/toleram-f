...
import { useAddRoundQuestionMutation, useUpdateTournamentQuestionMutation } from '../../../app/Services/Admin/AdminTournament';
import { useParams } from "react-router-dom";

export default function CreateNewQuestionPopup({ open, setOpen, question, round }) {
  const { roundId } = useParams();
console.log(round)
  const [addQuestion, { isLoading: isAddQuestionLoading }] = useAddRoundQuestionMutation();
  const [updateQuestion, { isLoading: isUpdateQuestionLoading }] = useUpdateTournamentQuestionMutation();
        ...
        <div className="flex min-h-full items-center justify-center p-4">
          <DialogPanel className="relative bg-gray-900 rounded-lg p-4 sm:p-6 w-full max-w-md mx-2">
            ...
            {question ? 'Edit Question' : 'Create New Question - '}
              <p className="text-gray-200 text-sm font-normal">round: {round?.round_name}</p>
            </DialogTitle>

...
const Dream11TeamSelector = () => {
  ...
  const [createTeam, { isLoading: creatTeamLoadin }] = useCreateDream11TeamMutation();
  ...
  const handleCreateTeam = async () => {
    console.log('Creating team with players:', selectedPlayers);
    if (isTeamValid() && captain && viceCaptain) {
      ...
      try {
        const response = await createTeam({teamData}).unwrap();
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        <div className="relative flex-1">
          <input
            ...
            onBlur={(e) => e.target.blur()} // Lose focus after typing
          />
  ...
  const NavigationBar = () => (
    <div className="flex justify-between items-center py-3 px-4 border-t bg-white">
      ...
      {step < 3 ? (
        ...
        >
          {step === 1 ? 'Select Captain' : 'Preview Team'}
